...
class Rental
    ...
    def actions 
        so = Substractor::SubPercent.new(price, 0.7, :owner)
        sp = Substractor::SubPercent.new(price, 0.3*0.5, :insurance)
        su = Substractor::SubUnit.new(100, rental_days, :assistance)
        factory = ActionFactory.new
        factory.add :driver, :debit, price

        Hash[Substractor.decompose(price, [so, sp, su], :drivy)].each do |kind, amount|
            factory.add kind, :credit, amount
        end
